...
## the elegant EPI version, where right min heap, at the start of the loop
## is either the same height or 1 element bigger than left max heap
def online_median(sequence: Iterator[float]) -> List[float]:
    left_max = []
    right_min = []
    result = []
    ...
    for s in sequence:
        heapq.heappush(left_max, -heapq.heappushpop(right_min, s))
